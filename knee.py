...
def classify_boundary(line1, line2, binary_slice, target_label, slice_ref):
    if target_label in [1,2]: # tmp fix!
        ...
        #dist1 = np.mean([(p[0]-origin_pt[0])**2+(p[1]-origin_pt[1])**2 for p in line1])
        #dist2 = np.mean([(p[0]-origin_pt[0])**2+(p[1]-origin_pt[1])**2 for p in line2])        
    ...
    # if target_label not in [1,2]:
    #     plot_lines(line1, [origin_pt, (xmin,ymin), (xmin,ymax), (xmax,ymin), (xmax,ymax)], line2, color='yellow')
...
roi_fname = r"E:\Data\doctor_xie\knee_data\QIU JIE-20150116\roi.nii.gz"
...
for slice_idx in z_nonzero_slices(roi):
    ...
    for target_label in candidate_label:
        ...
        for binary_slice in binary_slices:
            ...
            tmp_slice = binary_closing(tmp_slice, generate_binary_structure(2,1), iterations=2).astype(np.int)
