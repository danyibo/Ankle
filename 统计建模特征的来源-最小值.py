...
feature_path = r"E:\Data\doctor tao\max_and_min\new\log_wave_original\selected_2\result\Mean\PCC\Relief_26\LDA\LDA_coef.csv"
feature = pd.read_csv(feature_path)
feature = list(feature["Unnamed: 0"])

store_path =r"E:\Data\doctor tao\max_and_min\new\log_wave_original\feature_index"
max_feature = []
min_feature = []
for i in feature:
    if i.split("_")[0] == "MAX":
...
for i in pd_min["min_feature_name"]:
    ...
    def get_min(self):
        ...
        for i in range(len(selected_feature_name)):
            ...
            for i in range(len(case_name)):
                value = list(result_1.iloc[i][1:]).index(min(list(result_1.iloc[i][1:])))
        ...
        pd_result = pd.DataFrame(data=zip(big_index_list[0], big_index_list[1], big_index_list[2],
                                          ...
                                          big_index_list[9], big_index_list[10], big_index_list[11],
                                          big_index_list[12], big_index_list[13], big_index_list[14],
                                          big_index_list[15], big_index_list[16]
                                          ), columns=selected_feature_name)
        pd_result.to_csv(os.path.join(store_path, "new_min_index.csv"), index=None)
...
result = pd.read_csv(os.path.join(store_path, "new_min_index.csv"))
